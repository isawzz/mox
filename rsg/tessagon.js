...
class ValueBlend {
	constructor(options = {}) {
		console.log('ValueBlend', options)
		this._initCorners(options);
		console.log('BINGO!!!! corners', this.corners)
	}
...
class TileGenerator extends ValueBlend {
	...
	constructor(tessagon, options = {}) {
		...
		super(options);
		console.log('...TileGenerator', tessagon, options, this.corners)
		this.tessagon = tessagon;
...
class GridTileGenerator extends TileGenerator {
	constructor(tessagon, kwargs = {}) {
		...
		console.log('...GridTileGenerator', tessagon, kwargs, this.corners, this.tiles)
	...
	initializeTiles(kwargs = {}) {
		...
		console.log('!!!!!!!!initializeTiles', this.u_num, this.v_num)
		for (let u = 0; u < this.u_num; u++) {
			...
			for (let v = 0; v < this.v_num; v++) {
				...
				assertion(false,`>>>>>${corners}`)

				tiles[u][v] = this.createTile(u, v, corners, ...Object.values(kwargs));
			}
		}

		this.tiles = tiles;
		console.log('HAAAAAAAAAAAAAAAAAAAAAAAtiles', tiles)
		return tiles;
	}
...
class ParallelogramTileGenerator extends TileGenerator {
	...
	fingerprintRange() {
		...
		const u_range = [];
		for (let i = uMin; i < uMax; i++) u_range.push(i);

		const v_range = [];
		for (let i = vMin; i < vMax; i++) v_range.push(i);
...
class TessagonMetadata {
	...
	constructor(kwargs = {}) {
		...
		this._numColorPatterns = kwargs.num_color_patterns || 0;
		this._classification = kwargs.classification || 'misc';
		this._shapes = kwargs.shapes || [];
		this._sides = kwargs.sides || [];
		this._uvRatio = kwargs.uv_ratio !== undefined ? kwargs.uv_ratio : null;
		this._extraParameters = kwargs.extra_parameters || {};
	}
	...
	get numColorPatterns() {
		return this._numColorPatterns;
	}

	get hasColorPatterns() {
		return this._numColorPatterns > 0;
	}
	...
	get humanReadableClassification() {
...
class Tessagon {
	static tileClass = null;
	static metadata = null;

	constructor(kwargs = {}) {
		console.log('YEAHHH', kwargs);

		if ('tile_generator' in kwargs) {
			this.tileGenerator = new kwargs.tile_generator(this, kwargs);
		} else if ('rot_factor' in kwargs) {
			// Deprecated?
			const rotFactor = kwargs.rot_factor;
			const extraArgs = {
				parallelogramVectors: [[rotFactor, -1], [1, rotFactor]]
			};
			this.tileGenerator = new ParallelogramTileGenerator(
				this,
				{ ...kwargs, ...extraArgs }
			);
		} else if ('parallelogram_vectors' in kwargs) {
			this.tileGenerator = new ParallelogramTileGenerator(this, kwargs);
		} else {
			this.tileGenerator = new GridTileGenerator(this, kwargs);
		}

		this._initializeFunction(kwargs);

		// Optional post processing function
		this.postProcess = kwargs.post_process || null;

		if ('adaptor_class' in kwargs) {
			const adaptorClass = kwargs.adaptor_class;
			// Extract adaptor options only if present in kwargs
			const adaptorOptions = {};
			if (adaptorClass.ADAPTOR_OPTIONS) {
				for (const option of adaptorClass.ADAPTOR_OPTIONS) {
					if (option in kwargs) {
						adaptorOptions[option] = kwargs[option];
					}
				}
			}
			this.meshAdaptor = new adaptorClass(adaptorOptions);
		} else {
			throw new Error('Must provide a mesh adaptor class');
		}

		this.colorPattern = kwargs.color_pattern || null;

		this.tiles = null;
		this.faceTypes = {};
		this.vertTypes = {};

		this._processExtraParameters(kwargs);
	}
	...
	createMesh() {
		...
		this.meshAdaptor.createEmptyMesh();

		this._calculateVerts();
		this._calculateFaces();

		if (this.colorPattern) {
			this._calculateColors();
		}

		this.meshAdaptor.finishMesh();
		...
		return this.meshAdaptor.getMesh();
	}

	inspect() {
		console.log(`\n=== ${this.constructor.name} ===\n`);
		for (let i = 0; i < this.tiles.length; i++) {
			this.tiles[i].inspect({ tileNumber: i });
		}
	}

	// Class getters similar to Python @classmethods
	static get numColorPatterns() {
		if (this.metadata == null) return 0;
		return this.metadata.numColorPatterns;
	}

	static get numExtraParameters() {
		if (this.metadata == null) return 0;
		return Object.keys(this.metadata.extraParameters).length;
	}

	// Protected methods

	_initializeFunction(kwargs) {
		this.f = null;

		if ('simple_2d' in kwargs) {
			let uMultiplier2d = 1.0;
			let vMultiplier2d = 1.0;
			if (this.constructor.metadata && this.constructor.metadata.uvRatio) {
				vMultiplier2d = 1.0 / this.constructor.metadata.uvRatio;
			}

			const tileAspect = this.tileGenerator.v_num / this.tileGenerator.u_num;
			const multiplier2d = kwargs.multiplier_2d || 1.0;
			uMultiplier2d *= multiplier2d;
			vMultiplier2d *= multiplier2d * tileAspect;

			const translate2d = kwargs.translate_2d || [0, 0];

			this.f = (u, v) => [
				translate2d[0] + uMultiplier2d * u,
				translate2d[1] + vMultiplier2d * v,
				0.0
			];
	...
	_initializeTiles() {
		console.log('...Tessagon:...initializeTiles', this.tileGenerator)
		this.tiles = this.tileGenerator.createTiles();
	}
	...
	_calculateColors() {
		this.meshAdaptor.initializeColors();
		for (const tile of this.tiles) {
			tile.calculateColors();
		}
	}

	_processExtraParameters(kwargs) {
		this.extraParameters = {};
		const parametersInfo = this.constructor.metadata?.extraParameters;
		if (!parametersInfo) return;

		for (const parameter in parametersInfo) {
			const parameterInfo = parametersInfo[parameter];
			if (!(parameter in kwargs)) continue;
			const value = kwargs[parameter];
			if (parameterInfo.type === 'float') {
				this._processFloatExtraParameter(parameter, value, parameterInfo);
			}
			// Add other types if needed
		}
	}

	_processFloatExtraParameter(parameter, value, parameterInfo) {
		const maxValue = parameterInfo.max;
		const minValue = parameterInfo.min;
		if (maxValue !== undefined && value > maxValue) {
			throw new Error(`Parameter ${parameter} (${value}) exceeds maximum (${maxValue})`);
		}
		if (minValue !== undefined && value < minValue) {
			throw new Error(`Parameter ${parameter} (${value}) below minimum (${minValue})`);
		}
		this.extraParameters[parameter] = value;
	}
}
...
class ListAdaptor extends BaseAdaptor {
	...
	initializeColors() {
		this.colorList = new Array(this.faceList.length).fill(0);
	}

	createVert(coords) {
		this.vertList.push(coords);
		return this.vertList.length - 1;
	}

	createFace(verts) {
		this.faceList.push(verts);
		return this.faceList.length - 1;
	}

	colorFace(face, colorIndex) {
		this.colorList[face] = colorIndex;
	}
...
class SvgAdaptor extends ListAdaptor {
	...
	constructor(kwargs = {}) {
		...
		this.svgRootTag = kwargs.svg_root_tag ?? false;
		this.style = kwargs.svg_style;
		this.svgFillColors = kwargs.svg_fill_colors;
		this.svgFillColor = kwargs.svg_fill_color;
		this.svgStrokeColor = kwargs.svg_stroke_color;
		this.svgStrokeWidth = kwargs.svg_stroke_width;
	}

	getMesh() {
		let buffer = "";
		if (this.svgRootTag) {
			if (this.svgRootTag === true) {
				buffer += '<svg xmlns="http://www.w3.org/2000/svg">\n';
			} else {
				buffer += this.svgRootTag;
			}
		}

		buffer += `<g${this.groupStyle()}>\n`;

		if (this.style) {
			buffer += `<style>${this.style}</style>`;
		}

		if (this.svgFillColors) {
			const colorIndices = this.makeColorIndices();
			for (let i = 0; i < this.svgFillColors.length; i++) {
				if (!(i in colorIndices)) continue;
				const fillColor = this.svgFillColors[i];
				const faces = colorIndices[i].map((j) => this.faceList[j]);
				buffer += this.makeColorGroup(faces, fillColor);
			}
		} else {
			for (const face of this.faceList) {
				buffer += this.makeFace(face);
			}
		}
	...
	groupStyle() {
		let style = "";
		if (this.svgStrokeColor) {
			style += `stroke:${this.svgStrokeColor};`;
		}
		if (this.svgStrokeWidth) {
			style += `stroke-width:${this.svgStrokeWidth};`;
		}
		if (this.svgFillColor) {
			style += `fill:${this.svgFillColor};`;
		}
		if (style.length > 0) {
			style = ` style="${style}"`;
		}
	...
	makeColorGroup(faces, fillColor) {
		let buffer = `<g style="fill:${fillColor};">\n`;
	...
	makeFace(face) {
		let buffer = '<polygon points="';
		buffer += face.map((v) => `${v[0]},${v[1]}`).join(" ");
		buffer += '" />\n';
		return buffer;
	}
}
